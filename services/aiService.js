...
/**
 * Claude tool definitions for favorites management
 */
...
/**
 * Claude tool definitions for word search in books (Gutenberg)
 */
...
/**
 * Claude tool definitions for arbitrary text analysis (via Code Execution)
 */
...
/**
 * Claude tool definitions for interactive visualization (via Code Execution)
 */
...
/**
 * Generate chat response using Claude with conversation history
 * Supports tool use for favorites management and book search
 * @param {string} message - User's message
 * @param {Array} conversationHistory - Array of previous messages in Claude format
...
async function generateChatResponse(message, conversationHistory = []) {
  try {
    ...
    if (toolUseBlocks.length > 0) {
      // Return tool calls for the controller to execute
      return {
        success: true,
    ...
    // Store the full content blocks for Claude API compatibility
 ...
 * Sends tool results back to Claude for natural language response
 * Also handles the case where Claude wants to call MORE tools after seeing results
 * @param {Array} conversationHistory - Conversation history up to tool call
 * @param {Object} assistantMessage - The assistant message object from Claude that requested tools
 * @param {Array} functionResults - Array of { id, name, result } objects
 * @returns {Promise<Object>} - { success: true, response: string, conversationHistory: Array, requiresFunctionExecution?: boolean, functionCalls?: Array, assistantMessage?: Object } or { error: string }
 */
async function continueAfterFunctionExecution(
  ...
  try {
    // Build the assistant message content for Claude
    ...
    // Build messages array with assistant message and tool results
    const messages = [
      ...conversationHistory,
    ...
    if (toolUseBlocks.length > 0) {
      // Claude wants another round of tool calls
      return {
        success: true,
        requiresFunctionExecution: true,
        functionCalls: toolUseBlocks.map((block) => ({
    ...
    console.error("Error continuing after tool execution:", error);
    return {
      error: error.message || "Failed to generate response. Please try again.",
    };
  }
